...
const TICKET_CATEGORY_ID = '1525166140299542751'; // 
...
const BANNER_URL = 'https://media.discordapp.net/attachments/1525200263173112018/1530635436660146317/image.png'; // Lien direct de ta bannière ticket
...
// Commandes par message (!panel et !panel-ticket)
client.on('messageCreate', async (message) => {
  ...
  // 2. Commande !panel-ticket (Panneau de support avec la bannière)
  if (message.content === '!panel-ticket') {
    ...
    const ticketEmbed = new EmbedBuilder()
      ...
      .setImage(BANNER_URL) // Ta bannière rouge "URGENCE LILLOISE TICKET"
...
client.on('interactionCreate', async (interaction) => {
  try {
    // 1. Sélection dans le menu déroulant -> Création du salon dans la catégorie
    if (interaction.isStringSelectMenu()) {
      if (interaction.customId === 'create_ticket_menu') {
        ...
        // Création du salon dans la catégorie spécifiée
        const ticketChannel = await guild.channels.create({
          name: `ticket-${member.user.username}`,
          type: 0, // GuildText
          parent: TICKET_CATEGORY_ID !== 'ID_DE_TA_CATEGORIE_ICI' ? TICKET_CATEGORY_ID : null,
    ...
    // 2. Boutons (Fermeture de ticket, Acceptation / Refus de recrutement)
